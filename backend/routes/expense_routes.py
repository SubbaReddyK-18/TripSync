...
def create(trip_id):
    ...
    db = get_db()
    budget = db["budgets"].find_one({"trip_id": ObjectId(trip_id)}, {"_id": 1})
    if not budget:
        return {"success": False, "error": {"code": "BUDGET_REQUIRED", "message": "Set a budget for this trip before adding expenses"}}, 400
