    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          {data?.budget ? (
            <button onClick={openUpdate} className="btn-secondary text-sm flex items-center gap-1.5">
      ...
      {data?.budget ? (
          ...
          <motion.div variants={fadeUp} className={`card border-2 ${data.health === 'over_budget' ? 'border-accent-red/20' : data.health === 'near_limit' || data.health === 'at_budget' ? 'border-accent-amber/20' : 'border-accent-green/20'}`}>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="md:col-span-1 space-y-2">
                ...
                <p className={`text-2xl font-bold ${health?.class || 'text-accent-green'}`}>{health?.icon} {health?.label}</p>
              </div>
              <div className="md:col-span-3 space-y-3">
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <p className="text-xs text-text-muted font-medium">Total Budget</p>
                    <p className="text-xl font-mono font-bold text-text-primary">₹{Math.round(data.total_budget / 100).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-text-muted font-medium">Spent</p>
                    <p className={`text-xl font-mono font-bold ${data.health === 'over_budget' ? 'text-accent-red' : 'text-accent-amber'}`}>₹{(data.total_spent / 100).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-text-muted font-medium">Remaining</p>
                    <p className={`text-xl font-mono font-bold ${data.health === 'over_budget' ? 'text-accent-red' : 'text-accent-green'}`}>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="w-full bg-primary-lighter rounded-full h-4 border border-border-light overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
              ...
              <div className="flex items-center justify-center">
                <ResponsiveContainer width="100%" height={240}>
                  <PieChart>
                    ...
                    ]} cx="50%" cy="50%" innerRadius={60} outerRadius={100} dataKey="value" startAngle={90} endAngle={-270}>
            ...
            <motion.div variants={fadeUp} className="card">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={data.daily_spending.map((d) => ({ ...d, amount: d.amount / 100 }))}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface-elevated)', border: '1px solid var(--color-border)', borderRadius: '8px' }} />
                  <Line type="monotone" dataKey="amount" stroke="#60a5fa" strokeWidth={2.5} dot={{ fill: '#60a5fa', r: 4 }} activeDot={{ r: 6, strokeWidth: 0 }} />
                </LineChart>
              </ResponsiveContainer>
            </motion.div>
          )}
                    ...
                    {history.map((h, i) => {
                      ...
                      return (
                        <div key={i} className="relative pl-10">
                          <div className="absolute left-[13px] top-1.5 w-3 h-3 rounded-full border-2 border-accent-green bg-surface" />
                          <div className="border border-border/30 rounded-xl p-4 bg-primary-lighter/20 hover:bg-primary-lighter/40 transition-colors">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card text-center py-20 border-dashed border-2 border-border/80">
      ...
      {showModal && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="modal-overlay" onClick={() => setShowModal(false)}>
          <motion.div initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.96, opacity: 0 }} className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-xl font-semibold mb-6">{isUpdate ? 'Update Budget' : 'Set Budget'}</h2>
