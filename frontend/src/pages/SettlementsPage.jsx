...
export default function SettlementsPage() {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4 lg:mb-6">
        <div>
          <h1 className="text-h3 lg:text-h4 text-text-primary">Settlements</h1>
          <p className="text-xs lg:text-sm text-text-muted mt-0">
            ...
            <div className="space-y-2 lg:space-y-3">
              {data.all.map((s) => {
                ...
                return (
                  ...
                  >
                    <div className={`absolute left-[12px] lg:left-[13px] top-[18px] lg:top-5 w-2.5 lg:w-3 h-2.5 lg:h-3 rounded-full border-2 ${isPending ? 'border-accent-amber' : 'border-accent-green'} bg-surface z-10`} />
                    <div className={`border rounded-xl p-3 lg:p-4 transition-colors ${isPending ? 'border-accent-amber/20 bg-accent-amber/5' : 'border-accent-green/20 bg-accent-green/5'}`}>
                      ...
                      <div className="mt-1.5 lg:mt-2 pt-1.5 lg:pt-2 border-t border-border/40 flex items-center justify-between text-[10px] lg:text-[11px] text-text-muted">
  ...
  function renderSection(title, direction, color, bgColor, borderColor, items, otherItems) {
    ...
    return (
      <div className={`card border ${borderColor}`}>
        <h3 className={`font-semibold ${color} mb-3 lg:mb-4 flex items-center gap-2 text-sm`}>
        ...
        {pending.length > 0 ? (
          ...
          <div>
            ...
            <p className="text-center text-[10px] lg:text-[11px] text-text-muted mt-1.5 lg:mt-2 font-medium">All settled — last settlement shown above</p>
          </div>
        ) : (
          <div className="text-center py-4 lg:py-6">
            <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-xl bg-primary-lighter flex items-center justify-center mx-auto mb-1.5 lg:mb-2 text-base lg:text-lg">✨</div>
            <p className="text-text-muted text-xs lg:text-sm">Nothing here yet</p>
          </div>
        )}
      </div>
    )
  ...
  function renderSettlementCard(s, direction) {
    ...
    const isPending = s.status === 'pending'
    return (
      <div key={s._id} className={`border rounded-xl p-3 lg:p-4 transition-all ${isPending ? 'border-accent-amber/20' : 'border-accent-green/20'} ${isPending ? 'hover:shadow-elevated' : ''}`}>
        ...
        <div className="mt-2 lg:mt-3 pt-2 lg:pt-3 border-t border-border/40 flex items-center justify-between text-[10px] lg:text-[11px] text-text-muted">
          <span>{s.status === 'settled' ? `Settled ${fmtTime(s.paid_at)}` : `Created ${fmtTime(s.created_at)}`}</span>
            {isPending && isIOwe && (
